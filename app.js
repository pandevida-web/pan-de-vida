...
function finishIntro(){
  ...
  introCar.style.left = "calc(100% - 66px)";
...
window.addEventListener("load", () => {
  ...
  const timer = setInterval(() => {
    ...
    introCar.style.left = `calc(${progress}% - ${Math.min(66, Math.max(16, progress * 0.66))}px)`;
...
/* =========================================================
   PAN DE VIDA V7 — navegación por secciones/pantallas
   ========================================================= */
(function initV7Navigation(){
  ...
  const valid=id=>views.some(v=>v.dataset.view===id);
  let current=null,changing=false;

  function setActiveNav(id){
    links.forEach(a=>a.classList.toggle("active-nav",a.dataset.navView===id));
  }

  function showView(id,instant=false){
    if(!valid(id))id="inicio";
    if(id===current&&!instant)return;
    if(changing&&!instant)return;

    const apply=()=>{
      views.forEach(v=>v.classList.toggle("active-view",v.dataset.view===id));
      current=id;
      setActiveNav(id);
  ...
  document.querySelectorAll('a[href^="#"]').forEach(link=>{
    ...
    if(!valid(id)||link.hasAttribute("data-nav-view"))return;
    link.addEventListener("click",e=>{
      e.preventDefault();
      showView(id);
    });
  ...
  showView(valid(initial)?initial:"inicio",true);
})();
